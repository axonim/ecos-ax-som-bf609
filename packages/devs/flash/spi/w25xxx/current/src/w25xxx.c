...
#ifdef CYGOPT_DEVS_FLASH_SPI_W25XXX_SUPPORT_DEBUG
...
#ifdef CYGOPT_DEVS_FLASH_SPI_W25XXX_SUPPORT_DEBUG
    #define FLASH_SPI_W25XXX_DEBUG(args...)     diag_printf(args)
#else
    #define FLASH_SPI_W25XXX_DEBUG(args...)
#endif
...
static const w25xxx_params w25xxx_supported_devices [] = {
  ...
  { // Support for Windbond W25Q32 32M-bit
    sector_size  : 16,
    sector_count : 1024,
    jedec_id     : 0x00EF4016
  },
  ...
  (struct cyg_flash_dev *dev)
{
  ...
  FLASH_SPI_W25XXX_DEBUG("device_id = 0x%X\n", device_id);
  ...
  FLASH_SPI_W25XXX_DEBUG("%s return\n", __FUNCTION__);
  return retval;
}
